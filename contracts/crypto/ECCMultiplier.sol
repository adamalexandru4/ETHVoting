...
library ECCMultiplier {
  ...
  function toBinaryBoolArray(uint256 n) public pure returns (bool[256] memory) {
    ...
    function multiply(uint256 d, uint[3] memory P) public pure returns (uint[3] memory) {
      uint[3] memory N = P;
      uint[3] memory Q;
      Q[0] = 1;
      Q[1] = 1;
    ...
    function shiftLeft(uint a, uint8 n) public pure returns (uint) {
        return a << n;
    }

    function isOne(uint a, uint8 n) public pure returns (bool) {
        return convertBytesToBytes32(toBytesEth(a)) & convertBytesToBytes32(toBytesEth(shiftLeft(0x01,n))) != 0;
    }

    function toBytesEth(uint256 x) public pure returns (bytes memory) {
    ...
    function convertBytesToBytes32(bytes memory inBytes) public pure returns (bytes32 outBytes32) {
        if (inBytes.length == 0) {
            return 0x0;
        }
        assembly {

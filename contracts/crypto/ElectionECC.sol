...
import "./ECCMultiplier.sol";

contract ElectionECC  {
  ...
  function writeBlindSig(address _voter, uint256 blindSig) public {
  ...
  function Vote(uint256 choiceCode, uint256 vote, uint hashVote, uint256 c, uint256 s) public {
    ...
    require(uint(keccak256(abi.encodePacked(uint(vote)))) == hashVote);
    votes[choiceCode]++;

    emit voteSuccess(msg.sender,hashVote);
  }

  function addEligibleVoter(address _voter) public {
    eligibleVoters[_voter].eligible = true;
  }

  function removeEligibleVoter(address _voter) public {
    eligibleVoters[_voter].eligible = false;
  }
  ...
  function verifyBlindSig(uint256 m, uint256 c, uint256 s) public returns(uint) {
